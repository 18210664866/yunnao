...
//上传书内容到七牛云
//function UpBooks($callBack = "image", $width = 00, $height = 100, $image = "") {
//    
//    echo '<iframe scrolling="no" frameborder="0" border="0" onload="this.height=this.contentWindow.document.body.scrollHeight;this.width=this.contentWindow.document.body.scrollWidth;" width=' . $width . ' height="' . $height . '"  src="' . U('Upload/uploadtext', array('Width' => $width, 'Height' => $height, 'BackCall' => $callBack)) . '"></iframe>
//         <input type="hidden" ' . 'value = "' . $image . '"' . 'name="' . $callBack . '" id="' . $callBack . '">';
//}
...
/*
 * 文件上传七牛云
 * @param  string    $rootPath   文件路径
 * $param  string    $savePath   文件保存目录
 */
function uploadText($rootPath,$savePath) {
